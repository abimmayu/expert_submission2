...
import '../../helpers/test_search_helpers.dart';
...
void main() {
  ...
  testWidgets('Should display circular progress indicator when loading',
    ...
    when(() => faketTvSearchBloc.state).thenReturn(SearchTvsLoading());

    final circularProgressIndicatorFinder =
        find.byType(CircularProgressIndicator);

    await tester
        .pumpWidget(_createTestableWidget(const SearchTelevisionPage()));
    await tester.pump();
  ...
  testWidgets(
      ...
      (WidgetTester tester) async {
    when(() => faketTvSearchBloc.state).thenReturn(SearchTvsLoaded(tvTestList));
    await tester
        .pumpWidget(_createTestableWidget(const SearchTelevisionPage()));
    await tester.pump();
  ...
  testWidgets('should display text with message when error',
    ...
    when(() => faketTvSearchBloc.state)
        .thenReturn(const SearchTvsError(errorMessage));
